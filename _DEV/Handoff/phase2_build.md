...
상세 절차는 `HANDOFF.md` §"다음 세션 시작 절차 (자이로 로그 수집 → 옵션 A 수정)" 참조. 요약:

1. Vercel 프리뷰 배포 완료 확인 (커밋 `646b4b9`).
...
`ArScene.spawnCreature` 또는 `CreatureLoader.load` 에서 `clip.tracks` 를 순회해 **최상위 본/노드의 `.position` 트랙만 제거**, rotation / scale / 하위 본 트랙은 유지.
...
*Phase 2 빌드 핸드오프 v1.1 — 2026-04-19 · 1차 실기 결과 + 2차 진단 덤프 기록 추가*
