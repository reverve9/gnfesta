# Phase 3 빌드 핸드오프 (R1·R2 완료·push + R3 코드 push 완료 · 체크포인트 ⓒ 검증 대기)

> **상태**: R1 완료·검증·push 완료. R2 구현·로컬 검증·체크포인트 ⓑ (5/5) 통과·push 완료. **R3 구현·로컬 검증(tsc+build)·push 완료 (2026-04-20, 커밋 `c969a55`)**. DB 마이그레이션 `0018` 원격 적용 · 체크포인트 ⓒ (§5-1~5-4) 사용자 검증 대기. 현재 main HEAD `c969a55`.
...
| **Phase 3-R3** | 서버 스폰 정책 재정의 + `movement_outlier_cap_m` 설정화 + Phase 3.5 격상·DEBUG 운영 정책 디폴트 유지 | 🟡 구현·로컬·push 완료 (2026-04-20, `c969a55`). DB 마이그레이션 `0018` 원격 적용 · 체크포인트 ⓒ 검증 대기 |
...
## 🟡 Phase 3-R3 구현·push 완료 상세 (2026-04-20, 체크포인트 ⓒ 검증 대기)
...
**선행 조치**: Supabase 에 `supabase/migrations/0018_ar_festival_movement_outlier.sql` 원격 적용. 기존 active row 는 ADD COLUMN DEFAULT 100 으로 자동 채워짐. seed 재실행은 `WHERE NOT EXISTS` 가드로 no-op (필요 없음).

**§5-1 DB 검증 (3쿼리)**

```sql
-- 1. 신규 필드 존재·초기값
SELECT movement_outlier_cap_m FROM ar_festival_settings WHERE active = true;
-- 기대: 100

-- 2. update_festival_settings 시그니처
SELECT proname, pg_get_function_arguments(oid) FROM pg_proc
WHERE proname = 'update_festival_settings';
-- 기대: 'p_settings jsonb, p_movement_outlier_cap_m integer DEFAULT NULL' 포함

-- 3. issue_spawn_token 본문
\sf issue_spawn_token
-- 기대: ar_festival_settings 조회 + capture_token_ttl_sec 사용 + 'cooldown_active:' 포맷
```

**§5-2 curl 검증 (4건)** — Production alias + Vercel Protection Bypass Token `ia5gBnX1MN3cP6AmxZfp36mwwaTkNzdh` 사용.

| # | 시나리오 | 기대 응답 |
|---|---|---|
| 1 | geofence 밖 좌표 (`37.7600, 128.9000`) | 403 `{ ok:false, reason:'outside_geofence', distance_m:<N> }` |
| 2 | geofence 안 (`37.7985, 128.8990`) + 첫 호출 | 200 `{ ok:true, spawn:{ token, creature_id, creature_rarity, expires_at, ... } }` |
| 3 | 어드민에서 cooldown 30 설정 → 즉시 재호출 | 429 `{ ok:false, reason:'cooldown', retry_after_sec:<N> }` |
| 4 | 어드민에서 TTL 120 설정 → 새 호출 | 200 · `expires_at - now ≈ 120s` |

테스트 후 `capture_cooldown_sec=0`, `capture_token_ttl_sec=60` 원복.

**§5-3 어드민 UI 왕복**

- `/admin/ar/settings` 스폰 스케줄 섹션에 "이동 이상치 상한 (m)" 필드 존재
- 100 → 50 저장 · 리로드 후 50 확인 · 100 원복 + 성공 토스트 정상

**§5-4 클라 E2E**

- `https://gnfesta.vercel.app/ar?debug=1` → 전화번호 입력 → PlayPage 진입
- DevPanel 에 `Outlier cap: 100m` 표시 확인
- 어드민에서 `movement_outlier_cap_m` 변경 후 **세션 재시작** (M-4 원칙 — 실시간 반영 미지원) 시 DevPanel 값 반영 확인
- (선택) geofence 밖으로 이동 → 다음 스폰 시도 시 `Server reject: outside_geofence · distance <N>m @ <time>` 노출 확인

### R3 부수 처리 기록

현재 없음. 필요 시 본 섹션에 추가 (M-7 이하로도 가능).
...
### ⏸ Phase 4 진입 체크리스트 (R3 체크포인트 ⓒ 통과 후 착수)

**포획 API 신규**:
- `/api/ar/capture` HTTP 엔드포인트 신설 (R3 까지 미존재)
- `capture_creature` RPC 재작성 (R1 에서 DROP 상태). geofence 기반 — 포획 시점의 `client_lat/lng` 를 다시 `ar_festival_settings.center_lat/lng/radius_m` 와 거리 비교하여 거절. `ar_spawn_tokens.consumed_at` 설정 + `ar_captures` INSERT + `ar_capture_attempts` 로그. 실패 result enum: `invalid_token` / `expired` / `duplicate` / `outside_geofence` / `velocity` 등
- **R3 에서 확정된 실패 응답 유니온(`outside_geofence` / `cooldown`) 은 포획 API 응답에도 동일 규약으로 반영** — 본 프로젝트 내 일관성 유지 (M-1 포획 UI 대체 경로 설계 시 참고)
...
**도감 UI**:
- `captured` 목록 + 미션 진척도(예: common 7/10, rare 2/3) 렌더링
- 기존 `CollectionPage.tsx` 는 placeholder 상태 — 실 데이터 연동

**PlayPage 포획 핸들러 M-1 해소**:
- 현재 `handleCanvasPointerDown` 의 로컬 captured state 전환 + `scheduler.markCaptured()` 만 있음
- Phase 4 에서 `/api/ar/capture` 호출 + 응답 분기 + 실패 reason UI 반영으로 교체
- 성공 시 `scheduler.markCaptured()` 유지, 실패 시 토큰 만료/쿨다운 등 사유별 분기

**M-5 (ArScene 인스턴스 누적) 이관 판단**:
- Phase 4 에서 포획 시 인스턴스 destroy 까지 묶을지, Phase 5 (에셋 도입) 로 이월할지는 Phase 4 프롬프트 작성 시 재판단
...
**ⓒ R3 사용자 검증 (🟡 대기, DB 마이그레이션 적용 + §5-1~5-4)**

| # | 영역 | 판정 | 비고 |
|---|---|---|---|
| §5-1 | DB 3건 (신규 필드 초기값 / update_festival_settings 시그니처 / issue_spawn_token 본문) | ⏸ 대기 | Supabase SQL Editor 에서 실행 |
| §5-2 | curl 4건 (outside_geofence 403 / 정상 200 / cooldown 429 / TTL 반영) | ⏸ 대기 | Vercel Protection Bypass Token 사용. 테스트 후 cooldown 0 / TTL 60 원복 |
| §5-3 | 어드민 UI 왕복 (필드 존재 / 100→50 / 100 원복) | ⏸ 대기 | `/admin/ar/settings` 스폰 스케줄 섹션 |
| §5-4 | 클라 E2E (DevPanel `Outlier cap` 표시 / 세션 재시작 시 반영) | ⏸ 대기 | `?debug=1` 진입. 선택: outside 이동 시 `Server reject` 행 확인 |
...
## 🔜 다음 단계 (R3 코드 push 완료 후 현재 위치 · 세션 인수인계)

현재 main HEAD `c969a55` (R3 코드 push 완료). Vercel Production 자동 배포는 push 즉시 트리거됨 — 다음 세션 시작 시 배포 상태 `vercel ls` 또는 Vercel 대시보드에서 확인.

### 다음 세션 재개 순서

1. ⏸ **DB 마이그레이션 원격 적용** — Supabase SQL Editor 또는 `supabase db push` 로 `supabase/migrations/0018_ar_festival_movement_outlier.sql` 적용. 기존 active row 는 ADD COLUMN DEFAULT 100 으로 자동 채워짐. seed 재실행은 `WHERE NOT EXISTS` 가드로 no-op — 추가 작업 불필요.
2. ⏸ **체크포인트 ⓒ 4영역 검증** — 본 문서 "R3 구현·push 완료 상세 > 체크포인트 ⓒ 사용자 검증 대기" 섹션의 §5-1 DB / §5-2 curl / §5-3 어드민 UI / §5-4 클라 E2E 순차 실행. curl 후 cooldown 0 / TTL 60 원복 필수.
3. ⏸ **검증 결과 수신 → 본 문서 v0.7 → v0.8 갱신** — 체크포인트 ⓒ 판정표(§5-1~5-4) 결과 반영. 부수 발견은 R3 부수 처리 기록 또는 M-7 이하 신규 메모로 추가.
4. 🚫 **Phase 4 착수 금지** — 체크포인트 ⓒ 통과 + Phase 4 프롬프트 수신 전까지 `/api/ar/capture` · `capture_creature` RPC 재작성 · 경품 미션 판정 · 도감 UI 구현 금지 (§2 범위 외).

### R3 가드 — 다음 세션에서도 유지

- ArScene · CreatureLoader · GyroController · CameraStream 불가침 (Phase 2 회귀 보호).
- 클라 훅 내부 구조 변경 금지 — settings 참조 전환 외 리팩토링은 R3 범위 아님.
- `useSpawnScheduler` "활성 currentSpawn TTL 중복방지" 로직 수정 금지 — 테스트 3 부분 통과 건은 Phase 7 QA 이관 방침 확정, R3/Phase 4 범위 아님.
- M-5 (ArScene 인스턴스 누적) / M-6 (PropertyBinding 경고 + 연속 동일 등급 포획 실패) 는 Phase 5 이관 그대로. Phase 3.5 격상 없음 확정.
- DEBUG 플래그 (`?debug=1` / `__ar_debug__`) 는 Phase 7 QA 까지 유지 — 제거/축소 금지.

### 체크포인트 ⓒ 미통과 시 대응 패턴

- **§5-1 SQL 검증 실패** — 마이그레이션 0018 적용이 누락됐거나 Supabase 캐시. SQL Editor 에서 `0018_ar_festival_movement_outlier.sql` 원문 재실행 (IF NOT EXISTS 가드로 멱등). 재실행 후에도 실패 시 `pg_catalog.pg_proc.prosrc` 확인.
- **§5-2 curl 2번(정상 200) 실패 — 500 server_error** — `get_festival_settings` RPC 반환이 null 인 경우. active row 존재 여부 재확인 → seed 미적용이면 `supabase/seeds/ar_festival_default.sql` 재적용 (WHERE NOT EXISTS 멱등).
- **§5-2 curl 3번(cooldown 429) 실패 — 200 반환** — 어드민 UI 에서 `capture_cooldown_sec=30` 저장이 반영됐는지 §5-1 쿼리 1 으로 재확인. DB 반영됐는데도 429 가 안 뜨면 RPC 본문 재적용 확인.
- **§5-4 DevPanel 표시 실패** — 새 세션에서 `?debug=1` 진입했는지, eager bootstrap (main.tsx) 이 실행됐는지 확인. `localStorage.__ar_debug__` 수동 set 도 가능 (627f327 설계).

### 필독 자료 (다음 세션)

- 본 문서 (`phase3_build.md` v0.7) — **최우선**. R3 완료 상세 + 체크포인트 ⓒ 검증 절차 전부 포함.
- `PHASE_3_R3_PROMPT.md` v1.0 — R3 착수 프롬프트 원문. 범위·결정 근거 참조.
- `phase3_redesign.md` v1.0 — 재설계 전제.
- `PHASE_3_R1_PROMPT.md` · `PHASE_3_R2_PROMPT.md` — 역사 (완료).
- 추후 Phase 4 프롬프트 — R3 체크포인트 ⓒ 통과 후 사용자 작성·전달 예정.
...
- 검증 전화번호: `01000000000` (기존 R1 검증과 동일 더미)

---

*Phase 3 빌드 핸드오프 v0.7 — R1·R2 완료·push + R3 코드 구현·로컬 검증·push 완료 (c969a55, 2026-04-20) · DB 0018 원격 적용 대기 · 체크포인트 ⓒ (§5-1~5-4) 사용자 검증 대기 · Phase 3.5 격상 없음 / DEBUG Phase 7 까지 유지 확정 · M-6 Phase 5 재검증 유지 — 2026-04-20*
