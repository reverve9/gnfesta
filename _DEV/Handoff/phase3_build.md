...
| Phase 1 DB 스키마 (`ar_*` 8 테이블) + RPC 5종 | 운영 Supabase 적용 완료 | 스키마 변경 최소화 — `ar_zones` 에 geofence 1 row 만 관리. 시간·이동량 스폰은 신규 테이블 또는 `ar_spawn_tokens` 확장 필요 (재설계 확정 대기) |
...
| 파일 | 수정 범위 |
|---|---|
| `src/features/ar/pages/PlayPage.tsx` | zone enter/leave 콜백 → geofence inside 체크로 단순화. 시간·이동량 기반 스폰 스케줄러 추가. 축제장 밖 안내 UI. 폴링 로직 재정의 |
| `_DEV/Handoff/phase3_build.md` (이 문서) | 재설계 완료 시 산출물 섹션 전면 재작성 |
...
*Phase 3 빌드 핸드오프 v0.2 (재설계 대기) — 2026-04-19*
